...
module.exports = async (req, res, next) => {
  try {
    ...
    if (!token) {
      return res
        .status(400)
        .json({ message: "Toekn not found in auth page backend" });
    }
    let decode;
    try {
    ...
    if (!decode) {
      return res
        .status(400)
        .json({ message: "Token not decoded in auth page backend" });
    }
    ...
    if (!existingUser) {
      return res
        .status(400)
        .json({ message: "User not found in db, auth page backend" });
    }
